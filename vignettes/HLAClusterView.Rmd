...
anti-tumor immunity [TODO Kumar2012].  
...
arthritis, and systemic lupus erythematosus [TODO Gutierrez-Arcelus2016]. 
...
it also presents challenges for organ transplantation [TODO Kumar2012]. 
Successful tissue and organ transplantation requires that donors and 
recipients have compatible HLA alleles[TODO Kumar2012]. Because of their 
...
This task is done by the *readHLADataset()* function.

```{r parseSample, warning=FALSE, message=FALSE}
HLAdata <- readHLADataset(hlaDbPath = "./Samples_HLA_typing.txt")
head(HLAdata)
```
